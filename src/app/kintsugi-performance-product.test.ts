...
describe("kintsugi product performance", () => {
  ...
  it("kintsugi perf: video export encodes loop-timed frames", () => {
    ...
    expect(plan.count).toBe(360);
    expect(plan.frameDurationMicros).toBe(33333);
    expect(getKintsugiVideoFramePlan(turntableLoopSeconds, 24).count).toBe(288);
    expect(getKintsugiVideoFramePlan(turntableLoopSeconds, 60).count).toBe(720);
